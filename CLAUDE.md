...
| `web`                   | app  | Next.js web frontend (App Router, `apps/frontend/web`, tags `web,scope:frontend`) — Chakra UI app shell (`@tc/ui`), mock-data-driven homepage/trip/destination browsing (`@tc/mocks`), Auth.js (`next-auth`) session handling; full auth flow wired to real `auth-service` through Kong via `@tc/client-api` (`src/lib/auth-service-client.ts` configures the client once) — routes live under the `src/app/(auth)/` route group (`login`, `register`, `verify-email`, `forgot-password`, `reset-password`; the group doesn't affect the URL, so paths stay `/login` etc.), each `page.tsx` rendering a form component from `src/components/auth/` (server actions colocated alongside as `<page>-actions.ts`); non-route components live under `src/components/<feature>/` (e.g. `src/components/shell/app-navbar.tsx`), never inside `src/app/` — account settings (change password) and other product pages (trips, profile) still pending |
