...
| `web`                   | app  | Next.js web frontend (App Router, `apps/frontend/web`, tags `web,scope:frontend`) — Chakra UI app shell (`@tc/ui`), mock-data-driven homepage/trip/destination browsing (`@tc/mocks`), Auth.js (`next-auth`) session handling; `/login` (`src/app/login/`) authenticates via NextAuth `Credentials` provider calling `signIn` from `@tc/client-api/services/auth-service` against real `auth-service` through Kong — see **Shared API SDK & wrapper libraries** below; other product pages (register, trips, profile) still pending |
