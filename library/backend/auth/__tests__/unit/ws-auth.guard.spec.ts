...
import * as authToken from '../../src/lib/auth.token';
...
describe('WsAuthGuard', () => {
    ...
    it('sets client.data.userId when the handshake auth token is valid', async () => {
        vi.spyOn(authToken, 'verifyAuthToken').mockResolvedValue({ userId: 'user-1' });
    ...
    it('falls back to the handshake query token', async () => {
        vi.spyOn(authToken, 'verifyAuthToken').mockResolvedValue({ sub: 'user-2' });
    ...
    it('throws WsException when the token is invalid', async () => {
        vi.spyOn(authToken, 'verifyAuthToken').mockRejectedValue(new Error('invalid'));
