...
| DB CLI scripts                                      | `library/backend/database/scripts/`                                                       | `database.datasource.ts`, bundled `run-migrations-entry.ts` → `dist/scripts/run-migrations.cjs`, bundled `seed-trips.ts` → `dist/scripts/seed-trips.cjs` (`bun run seed:trips`) |
...
| Generated `ServerApi` interservice client           | `library/backend/server-api/src/client.service.ts` + `src/types/<service>.ts` (committed) | `bun nx run server-api:api:generate`                                                                                                                                            |
...
When one microservice calls another directly (not through the gateway) on behalf of the current request — e.g. `trips-service` calling `messaging-service`/`notifications-service`, or `users-service` calling `trips-service` — inject `ServerApi` from `@tc/server-api` and call the target service through its getter (`.authService`, `.usersService`, `.tripsService`, `.messagingService`, `.notificationsService`), not `@tc/common`'s `HttpClient`, and never import `@tc/api-sdk` directly (module boundaries block it — see **Module boundaries** below). Import `ServerApiModule.forRoot()` into the calling service's `AppModule` (alongside `AuthModule.forRoot(...)`) so `ServerApi` is available for injection. Each `ServerApi` getter already points at the target service's own `*_SERVICE_URL` (`/api/v1`-suffixed in `@tc/config`'s `EnvSchema`, since the generated SDK paths don't include the global prefix — `app.setGlobalPrefix('api')` + URI versioning add it at runtime, but `generateOpenApiDocument` emits paths without it) — forward the caller's `Authorization` header instead of inventing a separate service credential. Thread it from controller → service → client:
...
export class NotificationsClient {
    ...
    async createNotification(payload: CreateNotificationPayload, authorization: string): Promise<void> {
        try {
            await this.serverApi.notificationsService.createNotification({ body: payload, headers: { Authorization: authorization } });
        } catch (error) {
...
- **`@tc/api-sdk`** (`library/shared/api-sdk`, tag `type:api-sdk`, no `scope:*` tag) — holds _all_ `hey-api`-generated code per backend microservice (types, fetch client, per-service class-based SDK operations, `@tanstack/react-query` hooks) and nothing else. It is a strict leaf (`onlyDependOnLibsWithTags: []`) and is **never imported by apps directly** — module boundaries only allow `scope:shared` projects (i.e. the two wrapper libs below) to depend on it.
- **`@tc/server-api`** (`library/backend/server-api`, tags `scope:shared` + `type:server-api`) — backend-only wrapper for interservice calls that bypass Kong. Exports a single injectable `ServerApi` class (registered via `ServerApiModule.forRoot()`) with one getter per service (`.authService`, `.usersService`, `.tripsService`, `.messagingService`, `.notificationsService`), each building a fresh `<Service>Sdk` (from `@tc/api-sdk/clients/<service>`) bound to that service's own `*_SERVICE_URL` (see **Internal service-to-service calls** above).
- **`@tc/client-api`** (`library/frontend/client-api`, tags `scope:shared` + `type:client-api`) — frontend-only wrapper for calls routed through the Kong gateway. Exports a single `ClientApi` class (constructor takes `ClientApiOptions`) with one getter per service (`.authService`, `.usersService`, `.tripsService`, `.messagingService`, `.notificationsService`), mirroring `ServerApi`'s shape. Each getter points that service's shared `hey-api` client at the Kong base URL and returns a fresh `<Service>Sdk`. Supports both cookie-based session auth (`credentials: 'include'`, the default) and header-based auth (`auth` option — a static token or async callback) simultaneously.
...
- **Per-service SDKs**: `library/shared/api-sdk/src/client.config.ts` runs `@hey-api/openapi-ts` once per `CLIENT_REGISTRY` entry against `apps/backend/<service>/openapi/<service>.openapi.json`, with the `@hey-api/typescript`, `@hey-api/sdk` (`operations.strategy: 'single'` with `containerName: '<PascalService>Sdk'`, so each service exports one `<Service>Sdk` class — e.g. `AuthServiceSdk`, `TripsServiceSdk` — via a static `__registry` hey-api uses internally to resolve the bound client for `@tanstack/react-query` hooks) and `@tanstack/react-query` plugins, emitting into `src/clients/<service>/` — a **separate output directory and client instance per service**, not merged into one combined client (an earlier merged-client design caused operationId collisions across services and got reverted). These generated files **are committed to git** (unlike the gitignored OpenAPI specs they're generated from).
- **`ServerApi` interservice client**: `library/backend/server-api/src/client.service.ts` is generated — not hand-written. `scripts/generate-server-apis.ts` renders the Handlebars template `scripts/templates/client.service.template.hbs` (driven by `CLIENT_REGISTRY`) into a single `@Injectable() ServerApi` class with one getter per service; each getter constructs that service's own `<Service>Sdk` (e.g. `AuthServiceSdk`) bound to a `createClient({ baseUrl, throwOnError: true })` client, with `baseUrl` read from `@nestjs/config`'s `ConfigService.get('<SERVICE>_SERVICE_URL')`. `library/backend/server-api/src/client.module.ts`'s `ServerApiModule.forRoot({ global? })` is a dynamic module (`global` defaults to `true`) registering `ServerApi` as a provider — import it into a service's `AppModule` alongside `AuthModule.forRoot(...)` to use it. The same script also renders `scripts/templates/client.types.template.hbs` into one `library/backend/server-api/src/types/<service>.ts` per service — a bare `export type * from '@tc/api-sdk/clients/<service>'` re-export, importable via the `@tc/server-api/types/<service>` subpath for request/response DTO types (the top-level `@tc/server-api` barrel only re-exports `ServerApi`/`ServerApiModule`/`ServerApiModuleOptions`, to avoid cross-service type name collisions).
- **`ClientApi` frontend client**: `library/frontend/client-api/src/client.ts` is generated — not hand-written, and there is no per-service `apis/` directory. `scripts/generate-client-apis.ts` renders `scripts/templates/client.template.hbs` (driven by `CLIENT_REGISTRY`) into a single `ClientApi` class with one getter per service, mirroring `ServerApi` almost exactly: each getter builds a fresh `createClient({ baseUrl, credentials, auth })` (from `@tc/api-sdk/clients/<service>/client`) using the constructor's `ClientApiOptions` and returns a fresh `<Service>Sdk` (e.g. `AuthServiceSdk`) bound to it — no shared/mutated singleton state. There's no NestJS `ConfigService` on the frontend (unlike `ServerApi`, which reads `*_SERVICE_URL` from it), so `ClientApi` takes `ClientApiOptions` via its constructor instead. The same script also renders `scripts/templates/client.types.template.hbs` into one `library/frontend/client-api/src/types/<service>.ts` per service — a bare `export type * from '@tc/api-sdk/clients/<service>'` re-export, importable via the `@tc/client-api/types/<service>` subpath (the top-level `@tc/client-api` barrel only re-exports `ClientApi`/`ClientApiOptions`, to avoid cross-service type name collisions — same reasoning as `@tc/server-api`'s `types/<service>` split).
- **Regenerate everything**: `bun run api:generate` (root) runs `nx run-many --target=api:generate`, which fans out to `server-api:api:generate` and `client-api:api:generate` — each `dependsOn: ["api-sdk:sdk:generate"]`, so `@tc/api-sdk` always regenerates first. Never hand-edit anything under `api-sdk/src/clients/<service>/`, `server-api/src/client.service.ts`, `server-api/src/types/`, `client-api/src/client.ts`, or `client-api/src/types/` — edit `client.registry.ts` or the relevant `.hbs` template instead and regenerate.
...
- **Backend usage**: `import { ServerApi, ServerApiModule } from '@tc/server-api';` — import `ServerApiModule.forRoot()` into the calling service's `AppModule`, then inject `ServerApi` into any provider and call e.g. `serverApi.notificationsService.createNotification({ body, headers })`; no manual `baseUrl` wiring needed, since each getter already points at its own service's `*_SERVICE_URL`.
- **Frontend usage**: `import { ClientApi } from '@tc/client-api';` — construct `new ClientApi({ baseUrl: \`${process.env.NEXT_PUBLIC_API_GATEWAY_URL}/api/v1\` })`once (e.g.`apps/frontend/web/src/lib/client-api.ts`) — appending `/api/v1`for the same reason as`server-api`(Kong's declarative routes and the generated SDK's operation paths both expect it;`NEXT_PUBLIC_API_GATEWAY_URL`itself, e.g.`http://localhost:8000`, is just the bare Kong origin) — then call e.g. `clientApi.tripsService.listMyTrips({ headers: { Authorization: \`Bearer ${accessToken}\` } })`, threading the caller's own per-request auth (NextAuth session token, etc.) as a call-level header, same as `server-api`callers forwarding`Authorization`. Reference: `apps/frontend/web/src/lib/auth-service-client.ts`and`trips-service-client.ts`bind SDK methods off the shared`clientApi`instance this way;`apps/frontend/web/src/auth.ts`calls`signIn`from`auth-service-client.ts`inside NextAuth's`Credentials`provider`authorize()`to back`/login` with real sign-in instead of mock data.
...
| `server-api`            | lib  | `library/backend/server-api` (tags `scope:shared`, `type:server-api`) — backend-only injectable `ServerApi` client (via `ServerApiModule.forRoot()`) with one getter per service, for interservice calls that bypass Kong; see **Shared API SDK & wrapper libraries** below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
bun run seed:trips                            # Seed general.trips with sample data (organizerId: seed-organizer-1, idempotent)
