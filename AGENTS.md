...
│       └── api-client/           # Generated hey-api SDK + Api wrapper class per backend service (see below)
...
| Generated per-service SDKs        | `library/shared/api-client/src/clients/<service>/` (committed)       | `bun nx run api-client:api:generate` (see **Shared API client** below)                                    |
| Generated `<Service>Api` wrappers | `library/shared/api-client/src/apis/<service>.api.ts` (committed)    | same `api-client:api:generate` target, second codegen step                                                |
| `api-client` codegen scripts      | `library/shared/api-client/scripts/`                                 | `client.config.ts` (hey-api codegen, in `src/`) + `generate-api-services.ts` (Handlebars template render) |
...
When one microservice calls another directly (not through the gateway) on behalf of the current request — e.g. `trips-service` calling `messaging-service`/`notifications-service`, or `users-service` calling `trips-service` — construct the target service's `<Service>Api` from `@tc/api-client` directly, not `@tc/common`'s `HttpClient`. Point it at the target service's own `*_SERVICE_URL`, appending `/api/v1` since the generated SDK paths don't include the global prefix (`app.setGlobalPrefix('api')` + URI versioning add it at runtime, but `generateOpenApiDocument` emits paths without it), and forward the caller's `Authorization` header instead of inventing a separate service credential. Thread it from controller → service → client:
...
### Shared API client (`@tc/api-client`)

`library/shared/api-client` (tags `scope:shared`) generates one typed SDK per backend microservice and wraps each in a small `<Service>Api` class. Currently consumed by backend services for direct interservice calls (see **Internal service-to-service calls** above) — no frontend app wires it up yet (a prior `apps/frontend/web` integration was removed while the library's shape was still changing).

- **Source of truth for the service list**: `library/shared/api-client/src/client.registry.ts` exports `CLIENT_REGISTRY`, an array of `{ name, endpoint }` (e.g. `{ name: 'auth-service', endpoint: process.env.AUTH_SERVICE_URL }`). Add a new domain here when its microservice gains an OpenAPI spec — both codegen steps below read from this array.
- **Per-service SDKs**: `library/shared/api-client/src/client.config.ts` runs `@hey-api/openapi-ts` once per `CLIENT_REGISTRY` entry against `apps/backend/<service>/openapi/<service>.openapi.json`, emitting a `<PascalService>Sdk` class + its own `createClient` factory into `src/clients/<service>/` — a **separate output directory and client instance per service**, not merged into one combined client (an earlier merged-client design caused operationId collisions across services and got reverted). These generated files **are committed to git** (unlike the gitignored OpenAPI specs they're generated from).
- **`<Service>Api` wrappers**: `src/apis/<service>.api.ts` is itself generated — not hand-written. `scripts/generate-api-services.ts` renders the Handlebars template `scripts/templates/api-service.template.hbs` (driven by `CLIENT_REGISTRY`) into one file per service — each a **plain class** (no NestJS `@Injectable()`, not wired through DI) named `<PascalService>Api extends <PascalService>Sdk`, constructed with `ApiClientOptions = { baseUrl?: string }`. When `baseUrl` is omitted it falls back to `ApiClientUtils.getServiceEndpoint(service)` (the raw `CLIENT_REGISTRY` endpoint — no `/api/v1` suffix), so callers needing the versioned API path pass an explicit `baseUrl` with that suffix appended (see the interservice pattern above). `src/apis/index.ts` is the generated barrel exporting all five `<Service>Api` classes.
- **Regenerate everything**: `bun nx run api-client:api:generate` runs both codegen steps in order — `src/client.config.ts` (hey-api SDK codegen; depends on `openapi:generate` having produced fresh specs) → `scripts/generate-api-services.ts` (re-render `apis/*.api.ts` + barrel). Never hand-edit anything under `src/clients/<service>/` or `src/apis/` — edit `client.registry.ts` or the `.hbs` template instead and regenerate.
- **`scripts/` vs `src/`**: only `src/` is built/published (`tsconfig.lib.json` includes `src/**/*.ts` but excludes `src/client.config.ts`); `client.config.ts` and `scripts/**` are dev-only codegen tooling, type-checked separately via `tsconfig.scripts.json` and excluded from the `@nx/dependency-checks` lint rule (`eslint.config.mjs` `ignoredFiles`) since their deps (`@hey-api/openapi-ts`, `handlebars`, `prettier`) aren't part of the runtime package.
- **Usage**: `import { NotificationsServiceApi } from '@tc/api-client'; const api = new NotificationsServiceApi({ baseUrl: 'http://localhost:3004/api/v1' }); await api.createNotification({ body, headers });` — construct directly wherever needed; there's no DI module (`ApiClientModule`/`ApiClientService` were removed, since each caller needs a different `baseUrl` per target service or gateway, which a bare NestJS provider can't express without a factory anyway).
- **Per-service subpath imports**: `@tc/api-client/clients/<service>` is also a valid import path — `package.json`'s `exports` maps `./clients/*` to `src/clients/*/index.ts` (via the `@touring.club/source` condition every project in this workspace resolves through) and to `dist/clients/*/index.{js,d.ts}` for a real build (the `build` target sets `bundle: false` so `dist/` mirrors `src/` per-file instead of one bundled `dist/index.js`). Use this to import a specific service's generated types directly, e.g. `import type { TravelHistoryResponseDto } from '@tc/api-client/clients/trips-service';`. The top-level barrel (`src/index.ts`) only re-exports `./apis` — a flat `export *` across all five `clients/<service>` modules collides on `ClientOptions`/`Options`, which hey-api hardcodes per service and can't rename via plugin config, so those types are only reachable through their own service's subpath, never the root import.
...
| `api-client`            | lib  | `library/shared/api-client` (tags `scope:shared`) — one generated `hey-api` SDK per backend service plus a generated `<Service>Api` wrapper class per service; used by backend services for direct interservice calls, no frontend wiring yet; see **Shared API client** below |
